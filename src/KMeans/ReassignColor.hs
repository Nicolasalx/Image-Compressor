...
findIndexClosestCentroid (first : remain) color currentI minI minD
    | distance < minD =
        findIndexClosestCentroid remain color (currentI + 1) currentI distance
    | otherwise =
        findIndexClosestCentroid remain color (currentI + 1) minI minD
    where
        distance = euclidianDistance first color
findIndexClosestCentroid [] _ _ minI _ = minI

findClosestCentroid :: [Centroid] -> Color -> Int
findClosestCentroid centroid color =
    findIndexClosestCentroid centroid color 0 0 100000.0

reassignColor :: ([Centroid], [Pixel]) -> [Pixel]
reassignColor (centroid, ((Pixel x y r g b _) : remain)) = (Pixel x y r g b
    (findClosestCentroid centroid (Color r g b))) :
        reassignColor (centroid, remain)
reassignColor (_, []) = []

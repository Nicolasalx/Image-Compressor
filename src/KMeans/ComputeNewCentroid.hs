...
sumColor ((Pixel _ _ r g b _) : remain) =
    addColor (createICent r g b) (sumColor remain)
...
computeNewCentroidHelper ((_ : remain), pixel) i =
    (computeAverageColor (getPixelByIndex pixel i)) :
        computeNewCentroidHelper (remain, pixel) (i + 1)
